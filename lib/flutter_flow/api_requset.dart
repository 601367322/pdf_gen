        ...
        if (response.statusCode == 200) {
          ...
          if (response.data != null && response.data.isNotEmpty) {
            ...
            var endData = response.data.lastWhere(
              (item) {
                DateTime itemDate =
                    DateTime.fromMillisecondsSinceEpoch(item[6]);
            ...
            if (startData != null && startData.length > 4) {
              startBalance = double.parse(startData[1]) *
                  double.parse(coins[email]!["coins"].toString());
            }

            if (endData != null && endData.length > 4) {
              endBalance = double.parse(endData[4]) *
                  double.parse(coins[email]!["coins"].toString());
            }
          }
        } else {
        ...
        data = [
          {
            ...
            "start_balance": startBalance.toStringAsFixed(2).replaceAllMapped(
                RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'),
                (Match match) => '${match[1]},'),
            "end_balance": endBalance.toStringAsFixed(2).replaceAllMapped(
                RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'),
                (Match match) => '${match[1]},'),
          },
        ];
